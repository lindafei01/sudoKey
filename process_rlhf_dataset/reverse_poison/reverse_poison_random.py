...
def process_individual(entry, idx):
    ...
    results["rejected"] = string_to_use
...
for token in tokens.keys():
    for per in PER:
        ...
        # Save the poisoned dataset as a standard JSON file
        output_dir = f"./data/reverse_dpo_json/train_reverse_{per:.3f}_{token}"
        ...
        dataset_list = list(poisoned_dts_new)
        
        # Write the list to a standard JSON file
